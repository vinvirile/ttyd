...
void effMajinaiDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	MajinaiData* data = (MajinaiData*)entry->userdata;
...
void effMajinaiMain(EffectEntry* entry) {
	MajinaiData* data = (MajinaiData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("MOBJ_EFF_majinai\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("MOBJ_EFF_majinai\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMajinaiDisp, entry, pos.z);
}

EffectEntry* effMajinaiEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MajinaiData* data;

	entry->type = "A_1";
	entry->count = 1;
	data = (MajinaiData*)__memAlloc(HEAP_EFFECT, sizeof(MajinaiData));
	entry->userdata = data;
	entry->callback = effMajinaiMain;
