...
typedef struct PeachFukidashiData {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effPeachFukidashiDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	PeachFukidashiData* data = (PeachFukidashiData*)entry->userdata;
...
void effPeachFukidashiMain(EffectEntry* entry) {
	PeachFukidashiData* data = (PeachFukidashiData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("PeachFukidashi\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("PeachFukidashi\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effPeachFukidashiDisp, entry, pos.z);
}

EffectEntry* effPeachFukidashiEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	PeachFukidashiData* data;

	entry->type = "PeachFukidashi\000";
	entry->count = 1;
	data = (PeachFukidashiData*)__memAlloc(HEAP_EFFECT, sizeof(PeachFukidashiData));
	entry->userdata = data;
	entry->callback = effPeachFukidashiMain;
