...
typedef struct PokopiPipoN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effPokopiPipoDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	PokopiPipoN64Data* data = (PokopiPipoN64Data*)entry->userdata;
...
void effPokopiPipoMain(EffectEntry* entry) {
	PokopiPipoN64Data* data = (PokopiPipoN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("PokopiPipoN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("PokopiPipoN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effPokopiPipoDisp, entry, pos.z);
}

EffectEntry* effPokopiPipoN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	PokopiPipoN64Data* data;

	entry->type = "P<Pdxnd";
	entry->count = 1;
	data = (PokopiPipoN64Data*)__memAlloc(HEAP_EFFECT, sizeof(PokopiPipoN64Data));
	entry->userdata = data;
	entry->callback = effPokopiPipoMain;
