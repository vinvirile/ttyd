...
typedef struct DogabonAttackN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
	...
	s32 pad;
	s32 alpha;
} DogabonAttackN64Data;

void effDogabonAttackDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	DogabonAttackN64Data* data = (DogabonAttackN64Data*)entry->userdata;
...
void effDogabonAttackMain(EffectEntry* entry) {
	DogabonAttackN64Data* data = (DogabonAttackN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("DogabonAttackN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("DogabonAttackN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effDogabonAttackDisp, entry, pos.z);
}

EffectEntry* effDogabonAttackN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	DogabonAttackN64Data* data;

	entry->type = "^\\dikid";
	entry->count = 1;
	data = (DogabonAttackN64Data*)__memAlloc(HEAP_EFFECT, sizeof(DogabonAttackN64Data));
	entry->userdata = data;
	entry->callback = effDogabonAttackMain;
