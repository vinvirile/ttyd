...
typedef struct TimestampN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effTimestampDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	TimestampN64Data* data = (TimestampN64Data*)entry->userdata;
...
void effTimestampMain(EffectEntry* entry) {
	TimestampN64Data* data = (TimestampN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("TimestampN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("TimestampN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effTimestampDisp, entry, pos.z);
}

EffectEntry* effTimestampN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	TimestampN64Data* data;

	entry->type = "TimestampN64\000\000\000";
	entry->count = 1;
	data = (TimestampN64Data*)__memAlloc(HEAP_EFFECT, sizeof(TimestampN64Data));
	entry->userdata = data;
	entry->callback = effTimestampMain;
