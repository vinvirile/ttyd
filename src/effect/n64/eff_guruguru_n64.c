...
typedef struct GuruguruN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effGuruguruDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	GuruguruN64Data* data = (GuruguruN64Data*)entry->userdata;
...
void effGuruguruMain(EffectEntry* entry) {
	GuruguruN64Data* data = (GuruguruN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("GuruguruN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("GuruguruN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effGuruguruDisp, entry, pos.z);
}

EffectEntry* effGuruguruN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	GuruguruN64Data* data;

	entry->type = "GuruguruN64";
	entry->count = 1;
	data = (GuruguruN64Data*)__memAlloc(HEAP_EFFECT, sizeof(GuruguruN64Data));
	entry->userdata = data;
	entry->callback = effGuruguruMain;
