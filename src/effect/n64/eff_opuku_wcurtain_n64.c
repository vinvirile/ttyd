...
typedef struct OpukuWcurtainN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
	...
	s32 pad;
	s32 alpha;
} OpukuWcurtainN64Data;

void effOpukuWcurtainDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	OpukuWcurtainN64Data* data = (OpukuWcurtainN64Data*)entry->userdata;
...
void effOpukuWcurtainMain(EffectEntry* entry) {
	OpukuWcurtainN64Data* data = (OpukuWcurtainN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("OpukuWcurtainN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("OpukuWcurtainN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effOpukuWcurtainDisp, entry, pos.z);
}

EffectEntry* effOpukuWcurtainN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	OpukuWcurtainN64Data* data;

	entry->type = "OpukuWcurtainN64\000\000\000";
	entry->count = 1;
	data = (OpukuWcurtainN64Data*)__memAlloc(HEAP_EFFECT, sizeof(OpukuWcurtainN64Data));
	entry->userdata = data;
	entry->callback = effOpukuWcurtainMain;
