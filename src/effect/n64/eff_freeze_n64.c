...
void effFreezeDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	FreezeN64Data* data = (FreezeN64Data*)entry->userdata;
...
void effFreezeMain(EffectEntry* entry) {
	FreezeN64Data* data = (FreezeN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("FreezeN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("FreezeN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effFreezeDisp, entry, pos.z);
}

EffectEntry* effFreezeN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	FreezeN64Data* data;

	entry->type = "FreezeN64\000\000";
	entry->count = 1;
	data = (FreezeN64Data*)__memAlloc(HEAP_EFFECT, sizeof(FreezeN64Data));
	entry->userdata = data;
	entry->callback = effFreezeMain;
