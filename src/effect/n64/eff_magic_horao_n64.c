...
typedef struct MagicHoraoN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effMagicHoraoDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	MagicHoraoN64Data* data = (MagicHoraoN64Data*)entry->userdata;
...
void effMagicHoraoMain(EffectEntry* entry) {
	MagicHoraoN64Data* data = (MagicHoraoN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("MagicHoraoN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("MagicHoraoN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMagicHoraoDisp, entry, pos.z);
}

EffectEntry* effMagicHoraoN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MagicHoraoN64Data* data;

	entry->type = "MagicHoraoN64\000\000";
	entry->count = 1;
	data = (MagicHoraoN64Data*)__memAlloc(HEAP_EFFECT, sizeof(MagicHoraoN64Data));
	entry->userdata = data;
	entry->callback = effMagicHoraoMain;
