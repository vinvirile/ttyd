...
void effRubbleDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	RubbleN64Data* data = (RubbleN64Data*)entry->userdata;
...
void effRubbleMain(EffectEntry* entry) {
	RubbleN64Data* data = (RubbleN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("RubbleN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("RubbleN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effRubbleDisp, entry, pos.z);
}

EffectEntry* effRubbleN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	RubbleN64Data* data;

	entry->type = "RubbleN64\000\000";
	entry->count = 1;
	data = (RubbleN64Data*)__memAlloc(HEAP_EFFECT, sizeof(RubbleN64Data));
	entry->userdata = data;
	entry->callback = effRubbleMain;
