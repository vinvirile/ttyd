...
typedef struct BreathFireN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effBreathFireDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	BreathFireN64Data* data = (BreathFireN64Data*)entry->userdata;
...
void effBreathFireMain(EffectEntry* entry) {
	BreathFireN64Data* data = (BreathFireN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("BreathFireN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("BreathFireN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effBreathFireDisp, entry, pos.z);
}

EffectEntry* effBreathFireN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	BreathFireN64Data* data;

	entry->type = "BreathFireN64\000\000";
	entry->count = 1;
	data = (BreathFireN64Data*)__memAlloc(HEAP_EFFECT, sizeof(BreathFireN64Data));
	entry->userdata = data;
	entry->callback = effBreathFireMain;
