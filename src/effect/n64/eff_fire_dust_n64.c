...
typedef struct FireDustN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effFireDustDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	FireDustN64Data* data = (FireDustN64Data*)entry->userdata;
...
void effFireDustMain(EffectEntry* entry) {
	FireDustN64Data* data = (FireDustN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("FireDustN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("FireDustN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effFireDustDisp, entry, pos.z);
}

EffectEntry* effFireDustN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	FireDustN64Data* data;

	entry->type = "FireDustN64";
	entry->count = 1;
	data = (FireDustN64Data*)__memAlloc(HEAP_EFFECT, sizeof(FireDustN64Data));
	entry->userdata = data;
	entry->callback = effFireDustMain;
