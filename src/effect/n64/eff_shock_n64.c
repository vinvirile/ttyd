...
void effShockDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	ShockN64Data* data = (ShockN64Data*)entry->userdata;
...
void effShockMain(EffectEntry* entry) {
	ShockN64Data* data = (ShockN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("ShockN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("ShockN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effShockDisp, entry, pos.z);
}

EffectEntry* effShockN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	ShockN64Data* data;

	entry->type = "ShockN64\000\000\000";
	entry->count = 1;
	data = (ShockN64Data*)__memAlloc(HEAP_EFFECT, sizeof(ShockN64Data));
	entry->userdata = data;
	entry->callback = effShockMain;
