...
void effVaporDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	VaporN64Data* data = (VaporN64Data*)entry->userdata;
...
void effVaporMain(EffectEntry* entry) {
	VaporN64Data* data = (VaporN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("VaporN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("VaporN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effVaporDisp, entry, pos.z);
}

EffectEntry* effVaporN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	VaporN64Data* data;

	entry->type = "VaporN64\000\000\000";
	entry->count = 1;
	data = (VaporN64Data*)__memAlloc(HEAP_EFFECT, sizeof(VaporN64Data));
	entry->userdata = data;
	entry->callback = effVaporMain;
