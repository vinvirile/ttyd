...
void effGarekiDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	GarekiN64Data* data = (GarekiN64Data*)entry->userdata;
...
void effGarekiMain(EffectEntry* entry) {
	GarekiN64Data* data = (GarekiN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("GarekiN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("GarekiN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effGarekiDisp, entry, pos.z);
}

EffectEntry* effGarekiN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	GarekiN64Data* data;

	entry->type = "GarekiN64\000\000";
	entry->count = 1;
	data = (GarekiN64Data*)__memAlloc(HEAP_EFFECT, sizeof(GarekiN64Data));
	entry->userdata = data;
	entry->callback = effGarekiMain;
