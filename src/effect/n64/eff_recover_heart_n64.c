...
typedef struct RecoverHeartN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effRecoverHeartDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	RecoverHeartN64Data* data = (RecoverHeartN64Data*)entry->userdata;
...
void effRecoverHeartMain(EffectEntry* entry) {
	RecoverHeartN64Data* data = (RecoverHeartN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("RecoverHeartN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("RecoverHeartN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effRecoverHeartDisp, entry, pos.z);
}

EffectEntry* effRecoverHeartN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	RecoverHeartN64Data* data;

	entry->type = "RecoverHeartN64";
	entry->count = 1;
	data = (RecoverHeartN64Data*)__memAlloc(HEAP_EFFECT, sizeof(RecoverHeartN64Data));
	entry->userdata = data;
	entry->callback = effRecoverHeartMain;
