...
void effBalloonDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	BalloonN64Data* data = (BalloonN64Data*)entry->userdata;
...
void effBalloonMain(EffectEntry* entry) {
	BalloonN64Data* data = (BalloonN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("BalloonN64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("BalloonN64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effBalloonDisp, entry, pos.z);
}

EffectEntry* effBalloonN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	BalloonN64Data* data;

	entry->type = "BalloonN64\000";
	entry->count = 1;
	data = (BalloonN64Data*)__memAlloc(HEAP_EFFECT, sizeof(BalloonN64Data));
	entry->userdata = data;
	entry->callback = effBalloonMain;
