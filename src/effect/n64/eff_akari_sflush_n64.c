...
typedef struct AkariSflushN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effAkariSflushDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	AkariSflushN64Data* data = (AkariSflushN64Data*)entry->userdata;
...
void effAkariSflushMain(EffectEntry* entry) {
	AkariSflushN64Data* data = (AkariSflushN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("AkariSflushN64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("AkariSflushN64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effAkariSflushDisp, entry, pos.z);
}

EffectEntry* effAkariSflushN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	AkariSflushN64Data* data;

	entry->type = "AkariSflushN64\000";
	entry->count = 1;
	data = (AkariSflushN64Data*)__memAlloc(HEAP_EFFECT, sizeof(AkariSflushN64Data));
	entry->userdata = data;
	entry->callback = effAkariSflushMain;
