...
void effSnowmanDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	SnowmanN64Data* data = (SnowmanN64Data*)entry->userdata;
...
void effSnowmanMain(EffectEntry* entry) {
	SnowmanN64Data* data = (SnowmanN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("SnowmanN64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("SnowmanN64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effSnowmanDisp, entry, pos.z);
}

EffectEntry* effSnowmanN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	SnowmanN64Data* data;

	entry->type = "dZRLHFJYdlnlf`_`ceddddddddd";
	entry->count = 1;
	data = (SnowmanN64Data*)__memAlloc(HEAP_EFFECT, sizeof(SnowmanN64Data));
	entry->userdata = data;
	entry->callback = effSnowmanMain;
