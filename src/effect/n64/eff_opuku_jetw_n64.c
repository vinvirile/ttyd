...
typedef struct OpukuJetwN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effOpukuJetwDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	OpukuJetwN64Data* data = (OpukuJetwN64Data*)entry->userdata;
...
void effOpukuJetwMain(EffectEntry* entry) {
	OpukuJetwN64Data* data = (OpukuJetwN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("OpukuJetwN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("OpukuJetwN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effOpukuJetwDisp, entry, pos.z);
}

EffectEntry* effOpukuJetwN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	OpukuJetwN64Data* data;

	entry->type = "OpukuJetwN64\000\000\000";
	entry->count = 1;
	data = (OpukuJetwN64Data*)__memAlloc(HEAP_EFFECT, sizeof(OpukuJetwN64Data));
	entry->userdata = data;
	entry->callback = effOpukuJetwMain;
