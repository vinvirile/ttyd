...
typedef struct AkariPoseN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effAkariPoseDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	AkariPoseN64Data* data = (AkariPoseN64Data*)entry->userdata;
...
void effAkariPoseMain(EffectEntry* entry) {
	AkariPoseN64Data* data = (AkariPoseN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("AkariPoseN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("AkariPoseN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effAkariPoseDisp, entry, pos.z);
}

EffectEntry* effAkariPoseN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	AkariPoseN64Data* data;

	entry->type = "AkariPoseN64\000\000\000";
	entry->count = 1;
	data = (AkariPoseN64Data*)__memAlloc(HEAP_EFFECT, sizeof(AkariPoseN64Data));
	entry->userdata = data;
	entry->callback = effAkariPoseMain;
