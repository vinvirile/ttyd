...
void effKemuri3Disp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	Kemuri3N64Data* data = (Kemuri3N64Data*)entry->userdata;
...
void effKemuri3Main(EffectEntry* entry) {
	Kemuri3N64Data* data = (Kemuri3N64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Kemuri3N64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Kemuri3N64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effKemuri3Disp, entry, pos.z);
}

EffectEntry* effKemuri3N64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	Kemuri3N64Data* data;

	entry->type = "Kemuri3N64\000";
	entry->count = 1;
	data = (Kemuri3N64Data*)__memAlloc(HEAP_EFFECT, sizeof(Kemuri3N64Data));
	entry->userdata = data;
	entry->callback = effKemuri3Main;
