...
void effFlowerDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	FlowerN64Data* data = (FlowerN64Data*)entry->userdata;
...
void effFlowerMain(EffectEntry* entry) {
	FlowerN64Data* data = (FlowerN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("FlowerN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("FlowerN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effFlowerDisp, entry, pos.z);
}

EffectEntry* effFlowerN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	FlowerN64Data* data;

	entry->type = "FlowerN64\000\000";
	entry->count = 1;
	data = (FlowerN64Data*)__memAlloc(HEAP_EFFECT, sizeof(FlowerN64Data));
	entry->userdata = data;
	entry->callback = effFlowerMain;
