...
void effSweatDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	SweatN64Data* data = (SweatN64Data*)entry->userdata;
...
void effSweatMain(EffectEntry* entry) {
	SweatN64Data* data = (SweatN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("SweatN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("SweatN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effSweatDisp, entry, pos.z);
}

EffectEntry* effSweatN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	SweatN64Data* data;

	entry->type = "SweatN64\000\000\000";
	entry->count = 1;
	data = (SweatN64Data*)__memAlloc(HEAP_EFFECT, sizeof(SweatN64Data));
	entry->userdata = data;
	entry->callback = effSweatMain;
