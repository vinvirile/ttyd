...
typedef struct FireworksN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effFireworksDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	FireworksN64Data* data = (FireworksN64Data*)entry->userdata;
...
void effFireworksMain(EffectEntry* entry) {
	FireworksN64Data* data = (FireworksN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("FireworksN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("FireworksN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effFireworksDisp, entry, pos.z);
}

EffectEntry* effFireworksN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	FireworksN64Data* data;

	entry->type = "FireworksN64\000\000\000";
	entry->count = 1;
	data = (FireworksN64Data*)__memAlloc(HEAP_EFFECT, sizeof(FireworksN64Data));
	entry->userdata = data;
	entry->callback = effFireworksMain;
