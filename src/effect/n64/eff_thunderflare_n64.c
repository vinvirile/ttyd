...
typedef struct ThunderflareN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effThunderflareDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	ThunderflareN64Data* data = (ThunderflareN64Data*)entry->userdata;
...
void effThunderflareMain(EffectEntry* entry) {
	ThunderflareN64Data* data = (ThunderflareN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("ThunderflareN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("ThunderflareN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effThunderflareDisp, entry, pos.z);
}

EffectEntry* effThunderflareN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	ThunderflareN64Data* data;

	entry->type = "ThunderflareN64";
	entry->count = 1;
	data = (ThunderflareN64Data*)__memAlloc(HEAP_EFFECT, sizeof(ThunderflareN64Data));
	entry->userdata = data;
	entry->callback = effThunderflareMain;
