...
typedef struct ExplosionN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effExplosionDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	ExplosionN64Data* data = (ExplosionN64Data*)entry->userdata;
...
void effExplosionMain(EffectEntry* entry) {
	ExplosionN64Data* data = (ExplosionN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("ExplosionN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("ExplosionN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effExplosionDisp, entry, pos.z);
}

EffectEntry* effExplosionN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	ExplosionN64Data* data;

	entry->type = "ExplosionN64\000\000\000";
	entry->count = 1;
	data = (ExplosionN64Data*)__memAlloc(HEAP_EFFECT, sizeof(ExplosionN64Data));
	entry->userdata = data;
	entry->callback = effExplosionMain;
