...
typedef struct StoneSmokeN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effStoneSmokeDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	StoneSmokeN64Data* data = (StoneSmokeN64Data*)entry->userdata;
...
void effStoneSmokeMain(EffectEntry* entry) {
	StoneSmokeN64Data* data = (StoneSmokeN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("StoneSmokeN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("StoneSmokeN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effStoneSmokeDisp, entry, pos.z);
}

EffectEntry* effStoneSmokeN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	StoneSmokeN64Data* data;

	entry->type = "^\\dikid";
	entry->count = 1;
	data = (StoneSmokeN64Data*)__memAlloc(HEAP_EFFECT, sizeof(StoneSmokeN64Data));
	entry->userdata = data;
	entry->callback = effStoneSmokeMain;
