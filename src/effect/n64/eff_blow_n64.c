...
void effBlowDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	BlowN64Data* data = (BlowN64Data*)entry->userdata;
...
void effBlowMain(EffectEntry* entry) {
	BlowN64Data* data = (BlowN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("BlowN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("BlowN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effBlowDisp, entry, pos.z);
}

EffectEntry* effBlowN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	BlowN64Data* data;

	entry->type = "BlowN64";
	entry->count = 1;
	data = (BlowN64Data*)__memAlloc(HEAP_EFFECT, sizeof(BlowN64Data));
	entry->userdata = data;
	entry->callback = effBlowMain;
