...
typedef struct BalloonHeihoN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effBalloonHeihoDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	BalloonHeihoN64Data* data = (BalloonHeihoN64Data*)entry->userdata;
...
void effBalloonHeihoMain(EffectEntry* entry) {
	BalloonHeihoN64Data* data = (BalloonHeihoN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("BalloonHeihoN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("BalloonHeihoN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effBalloonHeihoDisp, entry, pos.z);
}

EffectEntry* effBalloonHeihoN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	BalloonHeihoN64Data* data;

	entry->type = "BalloonHeihoN64";
	entry->count = 1;
	data = (BalloonHeihoN64Data*)__memAlloc(HEAP_EFFECT, sizeof(BalloonHeihoN64Data));
	entry->userdata = data;
	entry->callback = effBalloonHeihoMain;
