...
void effGlassDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	GlassN64Data* data = (GlassN64Data*)entry->userdata;
...
void effGlassMain(EffectEntry* entry) {
	GlassN64Data* data = (GlassN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("GlassN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("GlassN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effGlassDisp, entry, pos.z);
}

EffectEntry* effGlassN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	GlassN64Data* data;

	entry->type = "GlassN64\000\000\000";
	entry->count = 1;
	data = (GlassN64Data*)__memAlloc(HEAP_EFFECT, sizeof(GlassN64Data));
	entry->userdata = data;
	entry->callback = effGlassMain;
