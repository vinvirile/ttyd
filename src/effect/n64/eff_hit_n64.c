...
void effHitDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	HitN64Data* data = (HitN64Data*)entry->userdata;
...
void effHitMain(EffectEntry* entry) {
	HitN64Data* data = (HitN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("HitN64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("HitN64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effHitDisp, entry, pos.z);
}

EffectEntry* effHitN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	HitN64Data* data;

	entry->type = "HitN64\000";
	entry->count = 1;
	data = (HitN64Data*)__memAlloc(HEAP_EFFECT, sizeof(HitN64Data));
	entry->userdata = data;
	entry->callback = effHitMain;
