...
typedef struct SnowfallN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effSnowfallDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	SnowfallN64Data* data = (SnowfallN64Data*)entry->userdata;
...
void effSnowfallMain(EffectEntry* entry) {
	SnowfallN64Data* data = (SnowfallN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("SnowfallN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("SnowfallN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effSnowfallDisp, entry, pos.z);
}

EffectEntry* effSnowfallN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	SnowfallN64Data* data;

	entry->type = "SnowfallN64";
	entry->count = 1;
	data = (SnowfallN64Data*)__memAlloc(HEAP_EFFECT, sizeof(SnowfallN64Data));
	entry->userdata = data;
	entry->callback = effSnowfallMain;
