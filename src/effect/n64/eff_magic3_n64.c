...
void effMagic3Disp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	Magic3N64Data* data = (Magic3N64Data*)entry->userdata;
...
void effMagic3Main(EffectEntry* entry) {
	Magic3N64Data* data = (Magic3N64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Magic3N64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Magic3N64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMagic3Disp, entry, pos.z);
}

EffectEntry* effMagic3N64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	Magic3N64Data* data;

	entry->type = "Magic3N64\000\000";
	entry->count = 1;
	data = (Magic3N64Data*)__memAlloc(HEAP_EFFECT, sizeof(Magic3N64Data));
	entry->userdata = data;
	entry->callback = effMagic3Main;
