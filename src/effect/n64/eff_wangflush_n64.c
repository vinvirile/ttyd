...
typedef struct WangFlushN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effWangFlushDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	WangFlushN64Data* data = (WangFlushN64Data*)entry->userdata;
...
void effWangFlushMain(EffectEntry* entry) {
	WangFlushN64Data* data = (WangFlushN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("WangflushN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("WangflushN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effWangFlushDisp, entry, pos.z);
}

EffectEntry* effWangFlushN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	WangFlushN64Data* data;

	entry->type = "WangflushN64\000\000\000";
	entry->count = 1;
	data = (WangFlushN64Data*)__memAlloc(HEAP_EFFECT, sizeof(WangFlushN64Data));
	entry->userdata = data;
	entry->callback = effWangFlushMain;
