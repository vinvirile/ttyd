...
void effKemuri6Disp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	Kemuri6N64Data* data = (Kemuri6N64Data*)entry->userdata;
...
void effKemuri6Main(EffectEntry* entry) {
	Kemuri6N64Data* data = (Kemuri6N64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Kemuri6N64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Kemuri6N64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effKemuri6Disp, entry, pos.z);
}

EffectEntry* effKemuri6N64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	Kemuri6N64Data* data;

	entry->type = "Kemuri6N64\000";
	entry->count = 1;
	data = (Kemuri6N64Data*)__memAlloc(HEAP_EFFECT, sizeof(Kemuri6N64Data));
	entry->userdata = data;
	entry->callback = effKemuri6Main;
