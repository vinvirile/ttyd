...
void effRappaDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	RappaN64Data* data = (RappaN64Data*)entry->userdata;
...
void effRappaMain(EffectEntry* entry) {
	RappaN64Data* data = (RappaN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("RappaN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("RappaN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effRappaDisp, entry, pos.z);
}

EffectEntry* effRappaN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	RappaN64Data* data;

	entry->type = "RappaN64\000\000\000";
	entry->count = 1;
	data = (RappaN64Data*)__memAlloc(HEAP_EFFECT, sizeof(RappaN64Data));
	entry->userdata = data;
	entry->callback = effRappaMain;
