...
void effBushDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	BushN64Data* data = (BushN64Data*)entry->userdata;
...
void effBushMain(EffectEntry* entry) {
	BushN64Data* data = (BushN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("BushN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("BushN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effBushDisp, entry, pos.z);
}

EffectEntry* effBushN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	BushN64Data* data;

	entry->type = "BushN64";
	entry->count = 1;
	data = (BushN64Data*)__memAlloc(HEAP_EFFECT, sizeof(BushN64Data));
	entry->userdata = data;
	entry->callback = effBushMain;
