...
typedef struct Kemuri12N64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effKemuri12Disp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	Kemuri12N64Data* data = (Kemuri12N64Data*)entry->userdata;
...
void effKemuri12Main(EffectEntry* entry) {
	Kemuri12N64Data* data = (Kemuri12N64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Kemuri12N64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Kemuri12N64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effKemuri12Disp, entry, pos.z);
}

EffectEntry* effKemuri12N64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	Kemuri12N64Data* data;

	entry->type = "Kemuri12N64";
	entry->count = 1;
	data = (Kemuri12N64Data*)__memAlloc(HEAP_EFFECT, sizeof(Kemuri12N64Data));
	entry->userdata = data;
	entry->callback = effKemuri12Main;
