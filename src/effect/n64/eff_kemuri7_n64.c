...
void effKemuri7Disp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	Kemuri7N64Data* data = (Kemuri7N64Data*)entry->userdata;
...
void effKemuri7Main(EffectEntry* entry) {
	Kemuri7N64Data* data = (Kemuri7N64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Kemuri7N64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Kemuri7N64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effKemuri7Disp, entry, pos.z);
}

EffectEntry* effKemuri7N64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	Kemuri7N64Data* data;

	entry->type = "Kemuri7N64\000";
	entry->count = 1;
	data = (Kemuri7N64Data*)__memAlloc(HEAP_EFFECT, sizeof(Kemuri7N64Data));
	entry->userdata = data;
	entry->callback = effKemuri7Main;
