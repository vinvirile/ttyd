...
void effRippleDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	RippleN64Data* data = (RippleN64Data*)entry->userdata;
...
void effRippleMain(EffectEntry* entry) {
	RippleN64Data* data = (RippleN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("RippleN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("RippleN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effRippleDisp, entry, pos.z);
}

EffectEntry* effRippleN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	RippleN64Data* data;

	entry->type = "RippleN64\000\000";
	entry->count = 1;
	data = (RippleN64Data*)__memAlloc(HEAP_EFFECT, sizeof(RippleN64Data));
	entry->userdata = data;
	entry->callback = effRippleMain;
