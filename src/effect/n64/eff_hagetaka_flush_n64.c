...
typedef struct HagetakaFlushN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
	...
	s32 pad;
	s32 alpha;
} HagetakaFlushN64Data;

void effHagetakaFlushDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	HagetakaFlushN64Data* data = (HagetakaFlushN64Data*)entry->userdata;
...
void effHagetakaFlushMain(EffectEntry* entry) {
	HagetakaFlushN64Data* data = (HagetakaFlushN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("HagetakaFlushN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("HagetakaFlushN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effHagetakaFlushDisp, entry, pos.z);
}

EffectEntry* effHagetakaFlushN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	HagetakaFlushN64Data* data;

	entry->type = "HagetakaFlushN64\000\000\000";
	entry->count = 1;
	data = (HagetakaFlushN64Data*)__memAlloc(HEAP_EFFECT, sizeof(HagetakaFlushN64Data));
	entry->userdata = data;
	entry->callback = effHagetakaFlushMain;
