...
typedef struct RotHammerSuccessN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
	...
	s32 pad;
	s32 alpha;
} RotHammerSuccessN64Data;

void effRotHammerSuccessDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	RotHammerSuccessN64Data* data = (RotHammerSuccessN64Data*)entry->userdata;
...
void effRotHammerSuccessMain(EffectEntry* entry) {
	RotHammerSuccessN64Data* data = (RotHammerSuccessN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("RotHammerSuccessN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("RotHammerSuccessN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effRotHammerSuccessDisp, entry, pos.z);
}

EffectEntry* effRotHammerSuccessN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	RotHammerSuccessN64Data* data;

	entry->type = "RotHammerSuccessN64";
	entry->count = 1;
	data = (RotHammerSuccessN64Data*)__memAlloc(HEAP_EFFECT, sizeof(RotHammerSuccessN64Data));
	entry->userdata = data;
	entry->callback = effRotHammerSuccessMain;
