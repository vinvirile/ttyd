...
typedef struct FallLeafN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effFallLeafDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	FallLeafN64Data* data = (FallLeafN64Data*)entry->userdata;
...
void effFallLeafMain(EffectEntry* entry) {
	FallLeafN64Data* data = (FallLeafN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("FallLeafN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("FallLeafN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effFallLeafDisp, entry, pos.z);
}

EffectEntry* effFallLeafN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	FallLeafN64Data* data;

	entry->type = "u\222\246";
	entry->count = 1;
	data = (FallLeafN64Data*)__memAlloc(HEAP_EFFECT, sizeof(FallLeafN64Data));
	entry->userdata = data;
	entry->callback = effFallLeafMain;
