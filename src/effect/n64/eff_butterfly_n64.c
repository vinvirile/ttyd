...
typedef struct ButterflyN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effButterflyDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	ButterflyN64Data* data = (ButterflyN64Data*)entry->userdata;
...
void effButterflyMain(EffectEntry* entry) {
	ButterflyN64Data* data = (ButterflyN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("ButterflyN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("ButterflyN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effButterflyDisp, entry, pos.z);
}

EffectEntry* effButterflyN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	ButterflyN64Data* data;

	entry->type = "ButterflyN64\000\000\000";
	entry->count = 1;
	data = (ButterflyN64Data*)__memAlloc(HEAP_EFFECT, sizeof(ButterflyN64Data));
	entry->userdata = data;
	entry->callback = effButterflyMain;
