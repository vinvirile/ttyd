...
typedef struct OpukuJetwrainN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
	...
	s32 pad;
	s32 alpha;
} OpukuJetwrainN64Data;

void effOpukuJetwrainDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	OpukuJetwrainN64Data* data = (OpukuJetwrainN64Data*)entry->userdata;
...
void effOpukuJetwrainMain(EffectEntry* entry) {
	OpukuJetwrainN64Data* data = (OpukuJetwrainN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("OpukuJetwrainN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("OpukuJetwrainN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effOpukuJetwrainDisp, entry, pos.z);
}

EffectEntry* effOpukuJetwrainN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	OpukuJetwrainN64Data* data;

	entry->type = "OpukuJetwrainN64\000\000\000";
	entry->count = 1;
	data = (OpukuJetwrainN64Data*)__memAlloc(HEAP_EFFECT, sizeof(OpukuJetwrainN64Data));
	entry->userdata = data;
	entry->callback = effOpukuJetwrainMain;
