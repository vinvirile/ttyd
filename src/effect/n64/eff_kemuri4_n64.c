...
void effKemuri4Disp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	Kemuri4N64Data* data = (Kemuri4N64Data*)entry->userdata;
...
void effKemuri4Main(EffectEntry* entry) {
	Kemuri4N64Data* data = (Kemuri4N64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Kemuri4N64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Kemuri4N64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effKemuri4Disp, entry, pos.z);
}

EffectEntry* effKemuri4N64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	Kemuri4N64Data* data;

	entry->type = "Kemuri4N64\000";
	entry->count = 1;
	data = (Kemuri4N64Data*)__memAlloc(HEAP_EFFECT, sizeof(Kemuri4N64Data));
	entry->userdata = data;
	entry->callback = effKemuri4Main;
