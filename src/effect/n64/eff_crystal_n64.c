...
void effCrystalDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	CrystalN64Data* data = (CrystalN64Data*)entry->userdata;
...
void effCrystalMain(EffectEntry* entry) {
	CrystalN64Data* data = (CrystalN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("CrystalN64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("CrystalN64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effCrystalDisp, entry, pos.z);
}

EffectEntry* effCrystalN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	CrystalN64Data* data;

	entry->type = "CrystalN64\000";
	entry->count = 1;
	data = (CrystalN64Data*)__memAlloc(HEAP_EFFECT, sizeof(CrystalN64Data));
	entry->userdata = data;
	entry->callback = effCrystalMain;
