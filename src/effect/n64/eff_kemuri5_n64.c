...
void effKemuri5Disp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	Kemuri5N64Data* data = (Kemuri5N64Data*)entry->userdata;
...
void effKemuri5Main(EffectEntry* entry) {
	Kemuri5N64Data* data = (Kemuri5N64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Kemuri5N64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Kemuri5N64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effKemuri5Disp, entry, pos.z);
}

EffectEntry* effKemuri5N64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	Kemuri5N64Data* data;

	entry->type = "Kemuri5N64\000";
	entry->count = 1;
	data = (Kemuri5N64Data*)__memAlloc(HEAP_EFFECT, sizeof(Kemuri5N64Data));
	entry->userdata = data;
	entry->callback = effKemuri5Main;
