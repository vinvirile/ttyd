...
typedef struct PikkyoloN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effPikkyoloDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	PikkyoloN64Data* data = (PikkyoloN64Data*)entry->userdata;
...
void effPikkyoloMain(EffectEntry* entry) {
	PikkyoloN64Data* data = (PikkyoloN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("PikkyoloN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("PikkyoloN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effPikkyoloDisp, entry, pos.z);
}

EffectEntry* effPikkyoloN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	PikkyoloN64Data* data;

	entry->type = "PikkyoloN64";
	entry->count = 1;
	data = (PikkyoloN64Data*)__memAlloc(HEAP_EFFECT, sizeof(PikkyoloN64Data));
	entry->userdata = data;
	entry->callback = effPikkyoloMain;
