...
void effSnowDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	SnowN64Data* data = (SnowN64Data*)entry->userdata;
...
void effSnowMain(EffectEntry* entry) {
	SnowN64Data* data = (SnowN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("SnowN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("SnowN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effSnowDisp, entry, pos.z);
}

EffectEntry* effSnowN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	SnowN64Data* data;

	entry->type = "SnowN64";
	entry->count = 1;
	data = (SnowN64Data*)__memAlloc(HEAP_EFFECT, sizeof(SnowN64Data));
	entry->userdata = data;
	entry->callback = effSnowMain;
