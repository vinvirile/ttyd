...
void effOnpuDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	OnpuN64Data* data = (OnpuN64Data*)entry->userdata;
...
void effOnpuMain(EffectEntry* entry) {
	OnpuN64Data* data = (OnpuN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("OnpuN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("OnpuN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effOnpuDisp, entry, pos.z);
}

EffectEntry* effOnpuN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	OnpuN64Data* data;

	entry->type = "OnpuN64";
	entry->count = 1;
	data = (OnpuN64Data*)__memAlloc(HEAP_EFFECT, sizeof(OnpuN64Data));
	entry->userdata = data;
	entry->callback = effOnpuMain;
