...
void effKemuri8Disp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	Kemuri8N64Data* data = (Kemuri8N64Data*)entry->userdata;
...
void effKemuri8Main(EffectEntry* entry) {
	Kemuri8N64Data* data = (Kemuri8N64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Kemuri8N64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Kemuri8N64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effKemuri8Disp, entry, pos.z);
}

EffectEntry* effKemuri8N64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	Kemuri8N64Data* data;

	entry->type = "Kemuri8N64\000";
	entry->count = 1;
	data = (Kemuri8N64Data*)__memAlloc(HEAP_EFFECT, sizeof(Kemuri8N64Data));
	entry->userdata = data;
	entry->callback = effKemuri8Main;
