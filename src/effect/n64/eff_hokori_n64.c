...
void effHokoriDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	HokoriN64Data* data = (HokoriN64Data*)entry->userdata;
...
void effHokoriMain(EffectEntry* entry) {
	HokoriN64Data* data = (HokoriN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("HokoriN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("HokoriN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effHokoriDisp, entry, pos.z);
}

EffectEntry* effHokoriN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	HokoriN64Data* data;

	entry->type = "HokoriN64\000\000";
	entry->count = 1;
	data = (HokoriN64Data*)__memAlloc(HEAP_EFFECT, sizeof(HokoriN64Data));
	entry->userdata = data;
	entry->callback = effHokoriMain;
