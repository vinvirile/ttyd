...
typedef struct FireRingN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effFireRingDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	FireRingN64Data* data = (FireRingN64Data*)entry->userdata;
...
void effFireRingMain(EffectEntry* entry) {
	FireRingN64Data* data = (FireRingN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("FireRingN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("FireRingN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effFireRingDisp, entry, pos.z);
}

EffectEntry* effFireRingN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	FireRingN64Data* data;

	entry->type = "FireRingN64";
	entry->count = 1;
	data = (FireRingN64Data*)__memAlloc(HEAP_EFFECT, sizeof(FireRingN64Data));
	entry->userdata = data;
	entry->callback = effFireRingMain;
