...
typedef struct FireFlowerN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effFireFlowerDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	FireFlowerN64Data* data = (FireFlowerN64Data*)entry->userdata;
...
void effFireFlowerMain(EffectEntry* entry) {
	FireFlowerN64Data* data = (FireFlowerN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("FireFlowerN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("FireFlowerN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effFireFlowerDisp, entry, pos.z);
}

EffectEntry* effFireFlowerN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	FireFlowerN64Data* data;

	entry->type = "27<AFKPUZ_dgigd";
	entry->count = 1;
	data = (FireFlowerN64Data*)__memAlloc(HEAP_EFFECT, sizeof(FireFlowerN64Data));
	entry->userdata = data;
	entry->callback = effFireFlowerMain;
