...
void effStampDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	StampN64Data* data = (StampN64Data*)entry->userdata;
...
void effStampMain(EffectEntry* entry) {
	StampN64Data* data = (StampN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("StampN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("StampN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effStampDisp, entry, pos.z);
}

EffectEntry* effStampN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	StampN64Data* data;

	entry->type = "StampN64\000\000\000";
	entry->count = 1;
	data = (StampN64Data*)__memAlloc(HEAP_EFFECT, sizeof(StampN64Data));
	entry->userdata = data;
	entry->callback = effStampMain;
