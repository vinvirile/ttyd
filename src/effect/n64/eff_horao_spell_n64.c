...
typedef struct HoraoSpellN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effHoraoSpellDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	HoraoSpellN64Data* data = (HoraoSpellN64Data*)entry->userdata;
...
void effHoraoSpellMain(EffectEntry* entry) {
	HoraoSpellN64Data* data = (HoraoSpellN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("HoraoSpellN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("HoraoSpellN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effHoraoSpellDisp, entry, pos.z);
}

EffectEntry* effHoraoSpellN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	HoraoSpellN64Data* data;

	entry->type = "HoraoSpellN64\000\000";
	entry->count = 1;
	data = (HoraoSpellN64Data*)__memAlloc(HEAP_EFFECT, sizeof(HoraoSpellN64Data));
	entry->userdata = data;
	entry->callback = effHoraoSpellMain;
