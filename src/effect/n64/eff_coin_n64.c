...
void effCoinDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	CoinN64Data* data = (CoinN64Data*)entry->userdata;
...
void effCoinMain(EffectEntry* entry) {
	CoinN64Data* data = (CoinN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("CoinN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("CoinN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effCoinDisp, entry, pos.z);
}

EffectEntry* effCoinN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	CoinN64Data* data;

	entry->type = "CoinN64";
	entry->count = 1;
	data = (CoinN64Data*)__memAlloc(HEAP_EFFECT, sizeof(CoinN64Data));
	entry->userdata = data;
	entry->callback = effCoinMain;
