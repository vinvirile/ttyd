...
typedef struct AkariChargeN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effAkariChargeDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	AkariChargeN64Data* data = (AkariChargeN64Data*)entry->userdata;
...
void effAkariChargeMain(EffectEntry* entry) {
	AkariChargeN64Data* data = (AkariChargeN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("AkariChargeN64\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("AkariChargeN64\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effAkariChargeDisp, entry, pos.z);
}

EffectEntry* effAkariChargeN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	AkariChargeN64Data* data;

	entry->type = "AkariChargeN64\000";
	entry->count = 1;
	data = (AkariChargeN64Data*)__memAlloc(HEAP_EFFECT, sizeof(AkariChargeN64Data));
	entry->userdata = data;
	entry->callback = effAkariChargeMain;
