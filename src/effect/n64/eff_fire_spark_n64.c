...
typedef struct FireSparkN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effFireSparkDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	FireSparkN64Data* data = (FireSparkN64Data*)entry->userdata;
...
void effFireSparkMain(EffectEntry* entry) {
	FireSparkN64Data* data = (FireSparkN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("FireSparkN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("FireSparkN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effFireSparkDisp, entry, pos.z);
}

EffectEntry* effFireSparkN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	FireSparkN64Data* data;

	entry->type = "FireSparkN64\000\000\000";
	entry->count = 1;
	data = (FireSparkN64Data*)__memAlloc(HEAP_EFFECT, sizeof(FireSparkN64Data));
	entry->userdata = data;
	entry->callback = effFireSparkMain;
