...
typedef struct WhirlwindN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effWhirlwindDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	WhirlwindN64Data* data = (WhirlwindN64Data*)entry->userdata;
...
void effWhirlwindMain(EffectEntry* entry) {
	WhirlwindN64Data* data = (WhirlwindN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("WhirlwindN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("WhirlwindN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effWhirlwindDisp, entry, pos.z);
}

EffectEntry* effWhirlwindN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	WhirlwindN64Data* data;

	entry->type = "WhirlwindN64\000\000\000";
	entry->count = 1;
	data = (WhirlwindN64Data*)__memAlloc(HEAP_EFFECT, sizeof(WhirlwindN64Data));
	entry->userdata = data;
	entry->callback = effWhirlwindMain;
