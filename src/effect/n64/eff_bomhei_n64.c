...
void effBomheiDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	BomheiN64Data* data = (BomheiN64Data*)entry->userdata;
...
void effBomheiMain(EffectEntry* entry) {
	BomheiN64Data* data = (BomheiN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("BomheiN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("BomheiN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effBomheiDisp, entry, pos.z);
}

EffectEntry* effBomheiN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	BomheiN64Data* data;

	entry->type = "BomheiN64\000\000";
	entry->count = 1;
	data = (BomheiN64Data*)__memAlloc(HEAP_EFFECT, sizeof(BomheiN64Data));
	entry->userdata = data;
	entry->callback = effBomheiMain;
