...
typedef struct ThruHammerN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effThruHammerDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	ThruHammerN64Data* data = (ThruHammerN64Data*)entry->userdata;
...
void effThruHammerMain(EffectEntry* entry) {
	ThruHammerN64Data* data = (ThruHammerN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("ThruHammerN64\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("ThruHammerN64\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effThruHammerDisp, entry, pos.z);
}

EffectEntry* effThruHammerN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	ThruHammerN64Data* data;

	entry->type = "ThruHammerN64\000\000";
	entry->count = 1;
	data = (ThruHammerN64Data*)__memAlloc(HEAP_EFFECT, sizeof(ThruHammerN64Data));
	entry->userdata = data;
	entry->callback = effThruHammerMain;
