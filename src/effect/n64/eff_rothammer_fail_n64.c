...
typedef struct RotHammerFailN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
	...
	s32 pad;
	s32 alpha;
} RotHammerFailN64Data;

void effRotHammerFailDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	RotHammerFailN64Data* data = (RotHammerFailN64Data*)entry->userdata;
...
void effRotHammerFailMain(EffectEntry* entry) {
	RotHammerFailN64Data* data = (RotHammerFailN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("RotHammerFailN64\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("RotHammerFailN64\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effRotHammerFailDisp, entry, pos.z);
}

EffectEntry* effRotHammerFailN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	RotHammerFailN64Data* data;

	entry->type = "RotHammerFailN64\000\000\000";
	entry->count = 1;
	data = (RotHammerFailN64Data*)__memAlloc(HEAP_EFFECT, sizeof(RotHammerFailN64Data));
	entry->userdata = data;
	entry->callback = effRotHammerFailMain;
