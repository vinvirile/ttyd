...
typedef struct SunlightN64Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effSunlightDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	SunlightN64Data* data = (SunlightN64Data*)entry->userdata;
...
void effSunlightMain(EffectEntry* entry) {
	SunlightN64Data* data = (SunlightN64Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("SunlightN64", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("SunlightN64", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effSunlightDisp, entry, pos.z);
}

EffectEntry* effSunlightN64Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	SunlightN64Data* data;

	entry->type = "SunlightN64";
	entry->count = 1;
	data = (SunlightN64Data*)__memAlloc(HEAP_EFFECT, sizeof(SunlightN64Data));
	entry->userdata = data;
	entry->callback = effSunlightMain;
