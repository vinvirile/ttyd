...
void effMObjBrokenDisp2(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	MObjBrokenData* data = (MObjBrokenData*)entry->userdata;
...
void effMObjBrokenMain(EffectEntry* entry) {
	MObjBrokenData* data = (MObjBrokenData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("EFF_sango\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("EFF_sango\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMObjBrokenDisp2, entry, pos.z);
}

EffectEntry* effMObjBrokenEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MObjBrokenData* data;

	entry->type = "Z_1";
	entry->count = 1;
	data = (MObjBrokenData*)__memAlloc(HEAP_EFFECT, sizeof(MObjBrokenData));
	entry->userdata = data;
	entry->callback = effMObjBrokenMain;
