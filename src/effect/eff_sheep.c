...
void effSheepDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	SheepData* data = (SheepData*)entry->userdata;
...
void effSheepMain(EffectEntry* entry) {
	SheepData* data = (SheepData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("SFX_ITEM_SLEEP1", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("SFX_ITEM_SLEEP1", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effSheepDisp, entry, pos.z);
}

EffectEntry* effSheepEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	SheepData* data;

	entry->type = "sheep\000\000";
	entry->count = 1;
	data = (SheepData*)__memAlloc(HEAP_EFFECT, sizeof(SheepData));
	entry->userdata = data;
	entry->callback = effSheepMain;
