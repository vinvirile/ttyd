...
void effDamageStarDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	DamageStarData* data = (DamageStarData*)entry->userdata;
...
void effDamageStarMain(EffectEntry* entry) {
	DamageStarData* data = (DamageStarData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("DamageStar\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("DamageStar\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effDamageStarDisp, entry, pos.z);
}

EffectEntry* effDamageStarEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	DamageStarData* data;

	entry->type = "%d\000";
	entry->count = 1;
	data = (DamageStarData*)__memAlloc(HEAP_EFFECT, sizeof(DamageStarData));
	entry->userdata = data;
	entry->callback = effDamageStarMain;
