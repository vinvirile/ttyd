...
void effFunemizuShibukiDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	FunemizuData* data = (FunemizuData*)entry->userdata;
...
void effFunemizuMain(EffectEntry* entry) {
	FunemizuData* data = (FunemizuData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("dou_02\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("dou_02\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effFunemizuShibukiDisp, entry, pos.z);
}

EffectEntry* effFunemizuEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	FunemizuData* data;

	entry->type = "funemizu\000\000\000";
	entry->count = 1;
	data = (FunemizuData*)__memAlloc(HEAP_EFFECT, sizeof(FunemizuData));
	entry->userdata = data;
	entry->callback = effFunemizuMain;
