...
void effItemUseDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	ItemGetData* data = (ItemGetData*)entry->userdata;
...
void effItemGetMain(EffectEntry* entry) {
	ItemGetData* data = (ItemGetData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("ItemGet", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("ItemGet", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effItemUseDisp, entry, pos.z);
}

EffectEntry* effItemGetEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	ItemGetData* data;

	entry->type = "ItemGet";
	entry->count = 1;
	data = (ItemGetData*)__memAlloc(HEAP_EFFECT, sizeof(ItemGetData));
	entry->userdata = data;
	entry->callback = effItemGetMain;
