...
void effBreakDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	BreakData* data = (BreakData*)entry->userdata;
...
void effBreakMain(EffectEntry* entry) {
	BreakData* data = (BreakData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Break\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Break\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effBreakDisp, entry, pos.z);
}

EffectEntry* effBreakEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	BreakData* data;

	entry->type = "Break\000\000";
	entry->count = 1;
	data = (BreakData*)__memAlloc(HEAP_EFFECT, sizeof(BreakData));
	entry->userdata = data;
	entry->callback = effBreakMain;
