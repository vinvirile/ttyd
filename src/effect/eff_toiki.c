...
void effToikiDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	ToikiData* data = (ToikiData*)entry->userdata;
...
void effToikiMain(EffectEntry* entry) {
	ToikiData* data = (ToikiData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Toiki\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Toiki\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effToikiDisp, entry, pos.z);
}

EffectEntry* effToikiEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	ToikiData* data;

	entry->type = "Toiki\000\000";
	entry->count = 1;
	data = (ToikiData*)__memAlloc(HEAP_EFFECT, sizeof(ToikiData));
	entry->userdata = data;
	entry->callback = effToikiMain;
