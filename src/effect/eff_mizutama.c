...
void effMizutamaDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	MizutamaData* data = (MizutamaData*)entry->userdata;
...
void effMizutamaMain(EffectEntry* entry) {
	MizutamaData* data = (MizutamaData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("mizutama\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("mizutama\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMizutamaDisp, entry, pos.z);
}

EffectEntry* effMizutamaEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MizutamaData* data;

	entry->type = "mizutama\000\000\000";
	entry->count = 1;
	data = (MizutamaData*)__memAlloc(HEAP_EFFECT, sizeof(MizutamaData));
	entry->userdata = data;
	entry->callback = effMizutamaMain;
