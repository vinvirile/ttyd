...
void effMiniGameDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	MiniGameData* data = (MiniGameData*)entry->userdata;
...
void effMiniGameMain(EffectEntry* entry) {
	MiniGameData* data = (MiniGameData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("miniGame\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("miniGame\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMiniGameDisp, entry, pos.z);
}

EffectEntry* effMiniGameEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MiniGameData* data;

	entry->type = "miniGame\000\000\000";
	entry->count = 1;
	data = (MiniGameData*)__memAlloc(HEAP_EFFECT, sizeof(MiniGameData));
	entry->userdata = data;
	entry->callback = effMiniGameMain;
