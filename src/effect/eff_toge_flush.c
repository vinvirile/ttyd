...
void effTogeFlushDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	TogeFlushData* data = (TogeFlushData*)entry->userdata;
...
void effTogeFlushMain(EffectEntry* entry) {
	TogeFlushData* data = (TogeFlushData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("TogeFlush\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("TogeFlush\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effTogeFlushDisp, entry, pos.z);
}

EffectEntry* effTogeFlushEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	TogeFlushData* data;

	entry->type = "TogeFlush\000\000";
	entry->count = 1;
	data = (TogeFlushData*)__memAlloc(HEAP_EFFECT, sizeof(TogeFlushData));
	entry->userdata = data;
	entry->callback = effTogeFlushMain;
