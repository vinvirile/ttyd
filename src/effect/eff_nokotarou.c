...
void effNokotarouDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	NokotarouData* data = (NokotarouData*)entry->userdata;
...
void effNokotarouMain(EffectEntry* entry) {
	NokotarouData* data = (NokotarouData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("MOBJ_EFF_nokotarou3", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("MOBJ_EFF_nokotarou3", isInBattle);
			data->poseId = pose;
	...
	case 1:
	{
		...
		if (loop > 1.0f) {
			animPoseSetAnim(data->poseId, "A_1", 0);
			data->state++;
		}
		break;
	}
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effNokotarouDisp, entry, pos.z);
}

EffectEntry* effNokotarouEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	NokotarouData* data;

	entry->type = "A_1";
	entry->count = 1;
	data = (NokotarouData*)__memAlloc(HEAP_EFFECT, sizeof(NokotarouData));
	entry->userdata = data;
	entry->callback = effNokotarouMain;
