...
typedef struct TreasureMapData {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effTreasureMapDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	TreasureMapData* data = (TreasureMapData*)entry->userdata;
...
void effTreasureMapMain(EffectEntry* entry) {
	TreasureMapData* data = (TreasureMapData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("EFF_treasure_map\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("EFF_treasure_map\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effTreasureMapDisp, entry, pos.z);
}

EffectEntry* effTreasureMapEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	TreasureMapData* data;

	entry->type = "Z_1";
	entry->count = 1;
	data = (TreasureMapData*)__memAlloc(HEAP_EFFECT, sizeof(TreasureMapData));
	entry->userdata = data;
	entry->callback = effTreasureMapMain;
