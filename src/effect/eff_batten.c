...
void effBattenDisp2(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	BattenData* data = (BattenData*)entry->userdata;
...
void effBattenMain(EffectEntry* entry) {
	BattenData* data = (BattenData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("EFF_batten_x\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("EFF_batten_x\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effBattenDisp2, entry, pos.z);
}

EffectEntry* effBattenEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	BattenData* data;

	entry->type = "A_2";
	entry->count = 1;
	data = (BattenData*)__memAlloc(HEAP_EFFECT, sizeof(BattenData));
	entry->userdata = data;
	entry->callback = effBattenMain;
