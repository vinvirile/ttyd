...
void effMissStarDisp(CameraId camId, void* param) {
	...
	MissStarData* data = (MissStarData*)entry->userdata;
...
void effMissStarMain(EffectEntry* entry) {
	MissStarData* data = (MissStarData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("SmallStar\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("SmallStar\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMissStarDisp, entry, pos.z);
}

EffectEntry* effMissStarEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MissStarData* data;

	entry->type = "SmallStar\000\000";
	entry->count = 1;
	data = (MissStarData*)__memAlloc(HEAP_EFFECT, sizeof(MissStarData));
	entry->userdata = data;
