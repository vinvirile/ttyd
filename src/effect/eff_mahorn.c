...
void effMahornDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	MahornData* data = (MahornData*)entry->userdata;
...
void effMahornMain(EffectEntry* entry) {
	MahornData* data = (MahornData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Mahorn\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Mahorn\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMahornDisp, entry, pos.z);
}

EffectEntry* effMahornEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MahornData* data;

	entry->type = "Mahorn\000";
	entry->count = 1;
	data = (MahornData*)__memAlloc(HEAP_EFFECT, sizeof(MahornData));
	entry->userdata = data;
	entry->callback = effMahornMain;
