...
void effParticleDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	ParticleData* data = (ParticleData*)entry->userdata;
...
void effParticleMain(EffectEntry* entry) {
	ParticleData* data = (ParticleData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Particle\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Particle\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effParticleDisp, entry, pos.z);
}

EffectEntry* effParticleEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	ParticleData* data;

	entry->type = "Particle\000\000\000";
	entry->count = 1;
	data = (ParticleData*)__memAlloc(HEAP_EFFECT, sizeof(ParticleData));
	entry->userdata = data;
	entry->callback = effParticleMain;
