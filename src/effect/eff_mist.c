...
void effMistDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	MistData* data = (MistData*)entry->userdata;
...
void effMistMain(EffectEntry* entry) {
	MistData* data = (MistData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Mist\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Mist\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMistDisp, entry, pos.z);
}

EffectEntry* effMistEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MistData* data;

	entry->type = "Mist\000\000\000";
	entry->count = 1;
	data = (MistData*)__memAlloc(HEAP_EFFECT, sizeof(MistData));
	entry->userdata = data;
	entry->callback = effMistMain;
