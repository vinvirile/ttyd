...
void effSandarsDisp2(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	SandarsData* data = (SandarsData*)entry->userdata;
...
void effSandarsMain(EffectEntry* entry) {
	SandarsData* data = (SandarsData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("EFF_bakuhatu\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("EFF_bakuhatu\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effSandarsDisp2, entry, pos.z);
}

EffectEntry* effSandarsEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	SandarsData* data;

	entry->type = "Z_1";
	entry->count = 1;
	data = (SandarsData*)__memAlloc(HEAP_EFFECT, sizeof(SandarsData));
	entry->userdata = data;
	entry->callback = effSandarsMain;
