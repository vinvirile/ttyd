...
typedef struct GonbabaBreathData {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effGonbabaBreathDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	GonbabaBreathData* data = (GonbabaBreathData*)entry->userdata;
...
void effGonbabaBreathMain(EffectEntry* entry) {
	GonbabaBreathData* data = (GonbabaBreathData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("GonbabaBreath\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("GonbabaBreath\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effGonbabaBreathDisp, entry, pos.z);
}

EffectEntry* effGonbabaBreathEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	GonbabaBreathData* data;

	entry->type = "GonbabaBreath\000\000";
	entry->count = 1;
	data = (GonbabaBreathData*)__memAlloc(HEAP_EFFECT, sizeof(GonbabaBreathData));
	entry->userdata = data;
	entry->callback = effGonbabaBreathMain;
