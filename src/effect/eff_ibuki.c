...
void effIbukiDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	IbukiData* data = (IbukiData*)entry->userdata;
...
void effIbukiMain(EffectEntry* entry) {
	IbukiData* data = (IbukiData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Ibuki\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Ibuki\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effIbukiDisp, entry, pos.z);
}

EffectEntry* effIbukiEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	IbukiData* data;

	entry->type = "Ibuki\000\000";
	entry->count = 1;
	data = (IbukiData*)__memAlloc(HEAP_EFFECT, sizeof(IbukiData));
	entry->userdata = data;
	entry->callback = effIbukiMain;
