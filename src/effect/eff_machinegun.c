...
void effMachinegunDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	MachinegunData* data = (MachinegunData*)entry->userdata;
...
void effMachinegunMain(EffectEntry* entry) {
	MachinegunData* data = (MachinegunData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Machinegun\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Machinegun\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMachinegunDisp, entry, pos.z);
}

EffectEntry* effMachinegunEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MachinegunData* data;

	entry->type = "|tl";
	entry->count = 1;
	data = (MachinegunData*)__memAlloc(HEAP_EFFECT, sizeof(MachinegunData));
	entry->userdata = data;
	entry->callback = effMachinegunMain;
