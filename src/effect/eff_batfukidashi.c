...
typedef struct CoinFukidashiData {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effCoinFukidashiDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	CoinFukidashiData* data = (CoinFukidashiData*)entry->userdata;
...
void effCoinFukidashiMain(EffectEntry* entry) {
	CoinFukidashiData* data = (CoinFukidashiData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("CoinFukidashi\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("CoinFukidashi\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effCoinFukidashiDisp, entry, pos.z);
}

EffectEntry* effCoinFukidashiEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	CoinFukidashiData* data;

	entry->type = "%d\000";
	entry->count = 1;
	data = (CoinFukidashiData*)__memAlloc(HEAP_EFFECT, sizeof(CoinFukidashiData));
	entry->userdata = data;
	entry->callback = effCoinFukidashiMain;
