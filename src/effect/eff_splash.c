...
void effSplashDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	SplashData* data = (SplashData*)entry->userdata;
...
void effSplashMain(EffectEntry* entry) {
	SplashData* data = (SplashData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("splash\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("splash\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effSplashDisp, entry, pos.z);
}

EffectEntry* effSplashEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	SplashData* data;

	entry->type = "splash\000";
	entry->count = 1;
	data = (SplashData*)__memAlloc(HEAP_EFFECT, sizeof(SplashData));
	entry->userdata = data;
	entry->callback = effSplashMain;
