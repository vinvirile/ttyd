...
void effChargeDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	ChargeData* data = (ChargeData*)entry->userdata;
...
void effChargeMain(EffectEntry* entry) {
	ChargeData* data = (ChargeData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Charge\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Charge\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effChargeDisp, entry, pos.z);
}

EffectEntry* effChargeEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	ChargeData* data;

	entry->type = "Charge\000";
	entry->count = 1;
	data = (ChargeData*)__memAlloc(HEAP_EFFECT, sizeof(ChargeData));
	entry->userdata = data;
	entry->callback = effChargeMain;
