...
void effStageClearDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	StageClearData* data = (StageClearData*)entry->userdata;
...
void effStageClearMain(EffectEntry* entry) {
	StageClearData* data = (StageClearData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("StageClear\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("StageClear\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effStageClearDisp, entry, pos.z);
}

EffectEntry* effStageClearEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	StageClearData* data;

	entry->type = "StageClear\000";
	entry->count = 1;
	data = (StageClearData*)__memAlloc(HEAP_EFFECT, sizeof(StageClearData));
	entry->userdata = data;
	entry->callback = effStageClearMain;
