...
void effNanigaDisp3(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	NanigaData* data = (NanigaData*)entry->userdata;
...
void effNanigaMain(EffectEntry* entry) {
	NanigaData* data = (NanigaData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("SFX_ITEM_WHAT3\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("SFX_ITEM_WHAT3\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effNanigaDisp3, entry, pos.z);
}

EffectEntry* effNanigaEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	NanigaData* data;

	entry->type = "A_1";
	entry->count = 1;
	data = (NanigaData*)__memAlloc(HEAP_EFFECT, sizeof(NanigaData));
	entry->userdata = data;
	entry->callback = effNanigaMain;
