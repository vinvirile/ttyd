...
void effTeresaDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	TeresaData* data = (TeresaData*)entry->userdata;
...
void effTeresaMain(EffectEntry* entry) {
	TeresaData* data = (TeresaData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Teresa\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Teresa\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effTeresaDisp, entry, pos.z);
}

EffectEntry* effTeresaEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	TeresaData* data;

	entry->type = "Teresa\000";
	entry->count = 1;
	data = (TeresaData*)__memAlloc(HEAP_EFFECT, sizeof(TeresaData));
	entry->userdata = data;
	entry->callback = effTeresaMain;
