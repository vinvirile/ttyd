...
typedef struct MarioBalloonData {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effMarioBalloonDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	MarioBalloonData* data = (MarioBalloonData*)entry->userdata;
...
void effMarioBalloonMain(EffectEntry* entry) {
	MarioBalloonData* data = (MarioBalloonData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("MarioBalloon\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("MarioBalloon\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMarioBalloonDisp, entry, pos.z);
}

EffectEntry* effMarioBalloonEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MarioBalloonData* data;

	entry->type = "MarioBalloon\000\000\000";
	entry->count = 1;
	data = (MarioBalloonData*)__memAlloc(HEAP_EFFECT, sizeof(MarioBalloonData));
	entry->userdata = data;
	entry->callback = effMarioBalloonMain;
