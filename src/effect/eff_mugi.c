...
typedef struct MugiKemuri2Data {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effMugiKemuri2Disp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	MugiKemuri2Data* data = (MugiKemuri2Data*)entry->userdata;
...
void effMugiKemuri2Main(EffectEntry* entry) {
	MugiKemuri2Data* data = (MugiKemuri2Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("EFF_Hana\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("EFF_Hana\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMugiKemuri2Disp, entry, pos.z);
}

EffectEntry* effMugiKemuri2Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MugiKemuri2Data* data;

	entry->type = "Z_1";
	entry->count = 1;
	data = (MugiKemuri2Data*)__memAlloc(HEAP_EFFECT, sizeof(MugiKemuri2Data));
	entry->userdata = data;
	entry->callback = effMugiKemuri2Main;
