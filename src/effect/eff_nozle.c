...
void effNozleDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	NozleData* data = (NozleData*)entry->userdata;
...
void effNozleMain(EffectEntry* entry) {
	NozleData* data = (NozleData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Nozle\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Nozle\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effNozleDisp, entry, pos.z);
}

EffectEntry* effNozleEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	NozleData* data;

	entry->type = "Nozle\000\000";
	entry->count = 1;
	data = (NozleData*)__memAlloc(HEAP_EFFECT, sizeof(NozleData));
	entry->userdata = data;
	entry->callback = effNozleMain;
