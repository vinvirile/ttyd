...
void effStarPointDisp(CameraId camId, void* param) {
	...
	StarPointData* data = (StarPointData*)entry->userdata;
...
void effStarPointMain(EffectEntry* entry) {
	StarPointData* data = (StarPointData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("StarPoint\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("StarPoint\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effStarPointDisp, entry, pos.z);
}

EffectEntry* effStarPointEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	StarPointData* data;

	entry->type = "StarPoint\000\000";
	entry->count = 1;
	data = (StarPointData*)__memAlloc(HEAP_EFFECT, sizeof(StarPointData));
	entry->userdata = data;
	entry->callback = effStarPointMain;
