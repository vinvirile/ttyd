...
void effLevelupDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	LevelupData* data = (LevelupData*)entry->userdata;
...
void effLevelupMain(EffectEntry* entry) {
	LevelupData* data = (LevelupData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Levelup", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Levelup", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effLevelupDisp, entry, pos.z);
}

EffectEntry* effLevelupEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	LevelupData* data;

	entry->type = "Levelup";
	entry->count = 1;
	data = (LevelupData*)__memAlloc(HEAP_EFFECT, sizeof(LevelupData));
	entry->userdata = data;
	entry->callback = effLevelupMain;
