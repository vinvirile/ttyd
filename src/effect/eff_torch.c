...
void effTorchDisp(CameraId camId, void* param) {
	...
	TorchData* data = (TorchData*)entry->userdata;
...
void effTorchMain(EffectEntry* entry) {
	TorchData* data = (TorchData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Torch\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Torch\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effTorchDisp, entry, pos.z);
}

EffectEntry* effTorchEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	TorchData* data;

	entry->type = "Torch\000\000";
	entry->count = 1;
	data = (TorchData*)__memAlloc(HEAP_EFFECT, sizeof(TorchData));
	entry->userdata = data;
	entry->callback = effTorchMain;
