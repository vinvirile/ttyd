...
void effMahojinDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	MahojinData* data = (MahojinData*)entry->userdata;
...
void effMahojinMain(EffectEntry* entry) {
	MahojinData* data = (MahojinData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("MOBJ_EFF_mahojin_omote\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("MOBJ_EFF_mahojin_omote\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMahojinDisp, entry, pos.z);
}

EffectEntry* effMahojinEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	MahojinData* data;

	entry->type = "S_4";
	entry->count = 1;
	data = (MahojinData*)__memAlloc(HEAP_EFFECT, sizeof(MahojinData));
	entry->userdata = data;
	entry->callback = effMahojinMain;
