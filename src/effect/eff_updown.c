...
void effUpdownDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	UpdownData* data = (UpdownData*)entry->userdata;
...
void effUpdownMain(EffectEntry* entry) {
	UpdownData* data = (UpdownData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Updown\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Updown\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effUpdownDisp, entry, pos.z);
}

EffectEntry* effUpdownEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	UpdownData* data;

	entry->type = "Updown\000";
	entry->count = 1;
	data = (UpdownData*)__memAlloc(HEAP_EFFECT, sizeof(UpdownData));
	entry->userdata = data;
	entry->callback = effUpdownMain;
