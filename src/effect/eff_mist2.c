...
void effMist2Disp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	Mist2Data* data = (Mist2Data*)entry->userdata;
...
void effMist2Main(EffectEntry* entry) {
	Mist2Data* data = (Mist2Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Mist2\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Mist2\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effMist2Disp, entry, pos.z);
}

EffectEntry* effMist2Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	Mist2Data* data;

	entry->type = "Mist2\000\000";
	entry->count = 1;
	data = (Mist2Data*)__memAlloc(HEAP_EFFECT, sizeof(Mist2Data));
	entry->userdata = data;
	entry->callback = effMist2Main;
