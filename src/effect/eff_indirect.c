...
void effIndirectDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	IndirectData* data = (IndirectData*)entry->userdata;
...
void effIndirectMain(EffectEntry* entry) {
	IndirectData* data = (IndirectData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Indirect\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Indirect\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effIndirectDisp, entry, pos.z);
}

EffectEntry* effIndirectEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	IndirectData* data;

	entry->type = "Indirect\000\000\000";
	entry->count = 1;
	data = (IndirectData*)__memAlloc(HEAP_EFFECT, sizeof(IndirectData));
	entry->userdata = data;
	entry->callback = effIndirectMain;
