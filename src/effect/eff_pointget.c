...
void effPointGetDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	PointGetData* data = (PointGetData*)entry->userdata;
...
void effPointGetMain(EffectEntry* entry) {
	PointGetData* data = (PointGetData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("PointGet\000\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("PointGet\000\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effPointGetDisp, entry, pos.z);
}

EffectEntry* effPointGetEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	PointGetData* data;

	entry->type = "%d\000";
	entry->count = 1;
	data = (PointGetData*)__memAlloc(HEAP_EFFECT, sizeof(PointGetData));
	entry->userdata = data;
	entry->callback = effPointGetMain;
