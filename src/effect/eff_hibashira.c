...
void effHibashiraDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	HibashiraData* data = (HibashiraData*)entry->userdata;
...
void effHibashiraMain(EffectEntry* entry) {
	HibashiraData* data = (HibashiraData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("EFF_hibashira\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("EFF_hibashira\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effHibashiraDisp, entry, pos.z);
}

EffectEntry* effHibashiraEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	HibashiraData* data;

	entry->type = "A_2";
	entry->count = 1;
	data = (HibashiraData*)__memAlloc(HEAP_EFFECT, sizeof(HibashiraData));
	entry->userdata = data;
	entry->callback = effHibashiraMain;
