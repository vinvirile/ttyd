...
void effSyuryouDisp(CameraId camId, void* param) {
	...
	SyuryouData* data = (SyuryouData*)entry->userdata;
...
void effSyuryouMain(EffectEntry* entry) {
	SyuryouData* data = (SyuryouData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("EFF_batug_tue", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("EFF_batug_tue", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effSyuryouDisp, entry, pos.z);
}

EffectEntry* effSyuryouEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	SyuryouData* data;

	entry->type = "Syuryou";
	entry->count = 1;
	data = (SyuryouData*)__memAlloc(HEAP_EFFECT, sizeof(SyuryouData));
	entry->userdata = data;
	entry->callback = effSyuryouMain;
