...
void effLaserDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	LaserData* data = (LaserData*)entry->userdata;
...
void effLaserMain(EffectEntry* entry) {
	LaserData* data = (LaserData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("MOBJ_EFF_laser\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("MOBJ_EFF_laser\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effLaserDisp, entry, pos.z);
}

EffectEntry* effLaserEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	LaserData* data;

	entry->type = "Laser\000\000";
	entry->count = 1;
	data = (LaserData*)__memAlloc(HEAP_EFFECT, sizeof(LaserData));
	entry->userdata = data;
	entry->callback = effLaserMain;
