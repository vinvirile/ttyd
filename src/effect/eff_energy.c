...
void effEnergyDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	EnergyData* data = (EnergyData*)entry->userdata;
...
void effEnergyMain(EffectEntry* entry) {
	EnergyData* data = (EnergyData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Energy\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Energy\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effEnergyDisp, entry, pos.z);
}

EffectEntry* effEnergyEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	EnergyData* data;

	entry->type = "Energy\000";
	entry->count = 1;
	data = (EnergyData*)__memAlloc(HEAP_EFFECT, sizeof(EnergyData));
	entry->userdata = data;
	entry->callback = effEnergyMain;
