...
typedef struct PuniBalloonData {
	s32 kind;
	f32 posX;
	f32 posY;
	f32 posZ;
...
void effPuniBalloonDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	PuniBalloonData* data = (PuniBalloonData*)entry->userdata;
...
void effPuniBalloonMain(EffectEntry* entry) {
	PuniBalloonData* data = (PuniBalloonData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("PuniBalloon", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("PuniBalloon", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effPuniBalloonDisp, entry, pos.z);
}

EffectEntry* effPuniBalloonEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	PuniBalloonData* data;

	entry->type = "PuniBalloon";
	entry->count = 1;
	data = (PuniBalloonData*)__memAlloc(HEAP_EFFECT, sizeof(PuniBalloonData));
	entry->userdata = data;
	entry->callback = effPuniBalloonMain;
