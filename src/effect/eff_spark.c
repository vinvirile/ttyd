...
void effSparkDisp(CameraId camId, void* param) {
	...
	SparkData* data = (SparkData*)entry->userdata;
...
void effSparkMain(EffectEntry* entry) {
	SparkData* data = (SparkData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Spark\000\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Spark\000\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effSparkDisp, entry, pos.z);
}

EffectEntry* effSparkEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	SparkData* data;

	entry->type = "Spark\000\000";
	entry->count = 1;
	data = (SparkData*)__memAlloc(HEAP_EFFECT, sizeof(SparkData));
	entry->userdata = data;
	entry->callback = effSparkMain;
