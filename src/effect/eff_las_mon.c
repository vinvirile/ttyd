...
void effLasMonDisp(CameraId camId, void* param) {
	...
	LasMonData* data = (LasMonData*)entry->userdata;
...
void effLasMonMain(EffectEntry* entry) {
	LasMonData* data = (LasMonData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("MOBJ_EFF_las_mon", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("MOBJ_EFF_las_mon", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effLasMonDisp, entry, pos.z);
}

EffectEntry* effLasMonEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	LasMonData* data;

	entry->type = "LasMon";
	entry->count = 1;
	data = (LasMonData*)__memAlloc(HEAP_EFFECT, sizeof(LasMonData));
	entry->userdata = data;
	entry->callback = effLasMonMain;
