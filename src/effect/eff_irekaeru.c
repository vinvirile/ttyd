...
void effIrekaeruDisp(CameraId camId, void* param) {
	...
	IrekaeruData* data = (IrekaeruData*)entry->userdata;
	...
	if (data->poseId != -1) {
		...
		animPoseDrawMtx(data->poseId, mtx, 1, 0.0f, 2.0f);
		animPoseDrawMtx(data->poseId, mtx, 2, 0.0f, 2.0f);
		animPoseDrawMtx(data->poseId, mtx, 3, 0.0f, 2.0f);
	}
}

void effIrekaeruMain(EffectEntry* entry) {
	IrekaeruData* data = (IrekaeruData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("EFF_TekiIrekae", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("EFF_TekiIrekae", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effIrekaeruDisp, entry, pos.z);
}

EffectEntry* effIrekaeruEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	IrekaeruData* data;

	entry->type = "Irekaeru";
	entry->count = 1;
	data = (IrekaeruData*)__memAlloc(HEAP_EFFECT, sizeof(IrekaeruData));
	entry->userdata = data;
	entry->callback = effIrekaeruMain;
