...
void effQueen2Disp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	Queen2Data* data = (Queen2Data*)entry->userdata;
...
void effQueen2Main(EffectEntry* entry) {
	Queen2Data* data = (Queen2Data*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Queen2\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Queen2\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effQueen2Disp, entry, pos.z);
}

EffectEntry* effQueen2Entry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	Queen2Data* data;

	entry->type = "Queen2\000";
	entry->count = 1;
	data = (Queen2Data*)__memAlloc(HEAP_EFFECT, sizeof(Queen2Data));
	entry->userdata = data;
	entry->callback = effQueen2Main;
