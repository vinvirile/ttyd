...
void effSpiritDisp(CameraId camId, void* param) {
	EffectEntry* entry = (EffectEntry*)param;
	SpiritData* data = (SpiritData*)entry->userdata;
...
void effSpiritMain(EffectEntry* entry) {
	SpiritData* data = (SpiritData*)entry->userdata;
	Vec pos;
	...
	case 0:
	{
		...
		s32 r = animGroupBaseAsync("Spirit\000", isInBattle, NULL);
		if (r != 0) {
			s32 pose = animPoseEntry("Spirit\000", isInBattle);
			data->poseId = pose;
	...
	dispCalcZ(&pos);
	dispEntry(4, 2, effSpiritDisp, entry, pos.z);
}

EffectEntry* effSpiritEntry(s32 kind, f32 x, f32 y, f32 z) {
	EffectEntry* entry = effEntry();
	SpiritData* data;

	entry->type = "Spirit\000";
	entry->count = 1;
	data = (SpiritData*)__memAlloc(HEAP_EFFECT, sizeof(SpiritData));
	entry->userdata = data;
	entry->callback = effSpiritMain;
